...
class TripsDetailsViewController: UITableViewController {
    ...
    //save change if the user modified the trip data
    @IBAction func saveTripInfo(_ sender: Any) {
        trip!.tripDate = tripDate.text!
        trip!.tripDuration = Int16(Int(tripDuration.text!)!)
        trip!.tripDestination = tripDestination.text!
    }
    
    //set the trip details as passed from the trip list view
